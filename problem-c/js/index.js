...
const extractOpponent = (game) => {
  return game.opponent === "UW" ? game.home : game.opponent;
};
...
let opponents =huskyGames2016.map(x => {
  return extractOpponent(x);
});
console.log(opponents);

//Define a function `huskiesLost()` that takes in a "game" object and returns
//whether or not UW lost.
const huskiesLost = (game) => {
  return game.opponent !== "UW" ? game.home_score < game.opponent_score : game.home_score > game.opponent_score;
};
...
lostGame.forEach(x => {
  let [opponentScore, uwScore] =
    x.opponent === "UW" ? [x.home_score, x.opponent_score] : [x.opponent_score, x.home_score];
  console.log(extractOpponent(x) + " at UW, " + opponentScore + " to " + uwScore);
});
...
console.log(huskyGames2016.filter(x => x.fumbles > 0).length);
...
const mostYardsPassing = (previous, current) => {
  return !previous.passing_yards ? current :
    (previous.passing_yards >= current.passing_yards ? previous : current);
};
...
const fumbledAndLostFilter = makeCombinedFilter(huskiesLost, x => x.fumbles > 0);
...
let fumbledAndLostGames = huskyGames2016.filter(x => fumbledAndLostFilter(x));
